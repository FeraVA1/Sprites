...
score_speedup = 100
game_speed = 8.0
skins = ["aqua","cobalt", "gold", "lime", "magma", "cobalt"]
names = ["Раушан", "Ильдан", "Руслан", "Вадим", "Ильнар", "Рамис"]
generation = 0
...
def run_game(genomes, config):
    ...
    while True:
        ...
        # очки и скорость игры
        score += 0.5 * (game_speed / 4)
        if score > score_speedup:
            score_speedup += 100 * (game_speed / 8)
            game_speed += 1
        ...
        # отображении имени динозавра
        for i, dinosaur in enumerate(dinosaurs):
            dname_label = dname_font.render(dinosaur.name, True, (212, 232, 30))
            dname_label_rect = dname_label.get_rect()
            dname_label_rect.center = (width - 100, 100 + (i * 25))
            screen.blit(dname_label, dname_label_rect)

        # отображение количества поколений
        label = heading_font.render("Поколоение: " + str(generation), True, (0, 72, 186))
